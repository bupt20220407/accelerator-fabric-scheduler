...
The topology-controller process also reconciles placement policies into ResourceClaimTemplates. Its project role can read placement policies and create, update, and delete templates. It does not create Pods or ResourceClaims. Namespace admission and quotas are still required before allowing untrusted users to request generated DRA templates.
...
The topology controller and DRA driver expose unauthenticated HTTP health and Prometheus endpoints on their Pod IPs. No Service, host port, or ingress publishes those ports. Production deployment requires NetworkPolicy and authenticated metrics collection. The scheduler retains its upstream authenticated HTTPS metrics endpoint.

W7 lets namespace users create PodGroups and label Pods into a group. The scheduler only reads PodGroups, but an untrusted user can intentionally create incomplete groups that occupy queue and Permit-waiting capacity. Production use requires namespace quotas, admission checks for PodGroup ownership/minMember/timeout, consistent member priority, and limits on pending gang size. The W7 kind configuration sets `podGroupRejectPercentage: 100` to exercise deterministic Permit timeout rollback; that setting disables optimistic PostFilter group rejection and must be reconsidered for production latency.
